...
| category | ENUM | - | 'culture' | 카테고리 (beach, mountain, culture, food, shopping, cafe) |
...
| role | ENUM | - | 'member' | 사용자 역할 (admin, vip, member) |
| status | ENUM | - | 'active' | 계정 상태 (active, inactive, suspended) |
...
| status | ENUM | - | 'pending' | 신고 상태 (pending, reviewed, resolved, dismissed) |
| created_at | TIMESTAMP | - | CURRENT_TIMESTAMP | 생성일시 |
| updated_at | TIMESTAMP | - | CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP | 수정일시 |
...
4. **검색 최적화 인덱스**: 자주 검색되는 컬럼 (category, status, is_approved 등)
