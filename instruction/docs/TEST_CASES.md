...
### 2.7 (헤더/개발용) 내 Role 변경

#### TC-USER-012: MEMBER → ADMIN Role 변경

- **목적**: 헤더/개발 편의 기능으로 본인 Role을 ADMIN으로 변경
- **전제조건**: 로그인된 상태(세션), 현재 roleCode = `MEMBER`
- **입력 데이터**:
...
- **예상 결과**:
    - HTTP 상태 코드: 200
    - `success: true`
    - `user.roleCode: "ADMIN"` 반환

#### TC-USER-013: ADMIN → MEMBER Role 변경

- **목적**: 본인 Role을 MEMBER로 되돌림
- **전제조건**: 로그인된 상태(세션), 현재 roleCode = `ADMIN`
- **입력 데이터**:
    - `POST /api/users/me/role`
    - Body:
    ```json
...
- **예상 결과**:
    - HTTP 상태 코드: 200
    - `success: true`
    - `user.roleCode: "MEMBER"` 반환

#### TC-USER-014: 비로그인 상태에서 Role 변경 시도

- **목적**: 인증이 필요한 엔드포인트에 비로그인으로 접근 시 실패 확인
- **전제조건**: 비로그인 상태(세션 없음)
- **입력 데이터**:
    - `POST /api/users/me/role`
- **예상 결과**:
    - HTTP 상태 코드: 401 (또는 `success: false` 응답)
    - 메시지: "로그인이 필요합니다."

---

## 2.x 사용자 프로필(타인) 페이지/화면 테스트케이스

#### TC-PROFILE-001: 리뷰/댓글에서 타인 프로필로 이동

- **목적**: 관광지 상세 페이지에서 리뷰/댓글 작성자 이름/프로필 이미지 클릭 시 타인 프로필로 이동
- **전제조건**: 특정 리뷰/댓글에 `userId`가 포함되어 있고, 작성자가 현재 로그인 사용자와 다름
- **입력 데이터**: UI 클릭
    - 리뷰 영역: 작성자 이름(`.userId`) 또는 프로필 이미지(`.comment-user-image`)
    - 댓글 영역: 작성자 이름(`.comment-user-name`) 또는 프로필 이미지(`.comment-user-image`)
    - 포토리뷰 모달: 작성자 이름(`.photo-review-modal-user`)
- **예상 결과**:
    - 브라우저가 `GET /pages/profile/{userId}`로 이동
    - 타인 프로필 페이지가 정상 렌더링됨

#### TC-PROFILE-002: 타인 프로필 페이지에서 기능 제한 확인

- **목적**: 타인 프로필에서 노출/비노출 요소가 올바른지 확인
- **전제조건**: 존재하는 타인 userId로 접근
- **입력 데이터**: `GET /pages/profile/2`
- **예상 결과**:
    - **프로필 수정 버튼이 숨김 처리됨**
    - **리뷰 탭만 노출됨** (댓글/좋아요/신청관리/관광지 추가신청 탭 숨김)
    - 리뷰 목록 로딩 시 `GET /api/users/2/reviews` 호출

#### TC-PROFILE-003: 본인 userId로 프로필 페이지 접근 시 마이페이지로 이동

- **목적**: `/pages/profile/{userId}`에 본인 userId로 접근하면 마이페이지로 정리되는지 확인
- **전제조건**: 로그인 상태, 본인 userId = 1
- **입력 데이터**: `GET /pages/profile/1`
- **예상 결과**:
    - `/pages/mypage/mypage`로 이동(리다이렉트/네비게이션)
...
#### TC-ADMIN-016: (UI) 사용자 관리 탭에서 프로필 버튼 동작

- **목적**: 관리자 페이지 사용자 관리 탭의 "프로필" 버튼으로 타인 프로필 페이지 접근
- **전제조건**: 관리자 로그인 상태, 사용자 목록이 표시됨
- **입력 데이터**: UI 클릭 (`프로필` 버튼)
- **예상 결과**:
    - 동일 탭에서 `GET /pages/profile/{userId}`로 이동
    - 타인 프로필 페이지가 정상 렌더링됨
...
| 1.1  | 2026-01-25 | -      | 타인 프로필 페이지(`/pages/profile/{userId}`) 및 이동(UI/관리자) 테스트케이스 추가 |
